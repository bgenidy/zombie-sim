...
public class AlphaZombie {
	...
	private final Color pink = new Color(1, 88, 115); //should be blue but got to lazy
	
	private boolean overRide;
	
	public AlphaZombie(ZombieModel model){
		this.model = model;
		dir = Direction.NORTH;
		overRide = false;
	}
	...
	public boolean init(int xlocation, int ylocation){
		...
		model.setColor(xlocation, ylocation, pink);
	...
	public void mouseClicked(int xloc, int yloc){
		xloc = xloc/model.getDotSize(); //setting x to relative size
		yloc = yloc/model.getDotSize(); //setting y to relative size
		
		int north = model.getHeight()/5;
		int south = model.getHeight() - north;
		int west = model.getWidth() / 5;
		int east = model.getWidth() - west;
		
		overRide = true; //the mouse has been clicked to override random gen
